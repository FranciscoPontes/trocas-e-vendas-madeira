...
export const login = (redirected) => {
  ...
  if (sessionStorage.getItem("cp-persuasive-user")) {
    const cachedCredential = JSON.parse(
      atob(sessionStorage.getItem("cp-persuasive-user"))
    );
    const credential = firebase.auth.GoogleAuthProvider.credential(
      cachedCredential.oauthIdToken,
      cachedCredential.oauthAccessToken
    );
  ...
  return firebase
    ...
    .then((result) => {
      sessionStorage.setItem(
        "cp-persuasive-user",
        btoa(JSON.stringify(result.credential))
      );
...
const getDocumentsOrdered = () =>
  db
    ...
    .then((response) => response)
    .catch((error) => console.error(error));
const getDocumentsOrderedCurrentUser = (filter) =>
  db
    .collection(SELLS_DATA)
    .where(filter.row, filter.comparator, filter.givenFilter)
    .orderBy("likeCount", "desc")
    .get()
    .then((response) => response)
    .catch((error) => console.error(error));
...
export const getBulkImageUrl = async (uId, images) =>
  await Promise.all(
    images.map(
      async (image) =>
        await getImageUrl(uId, image)
          .then((response) => response)
          .catch((error) => console.error(error))
    )
  );

const addImagesToData = async (
  data,
  uId = null,
  limit = false,
  likeList = null
  ...
  await data.docs.reduce(async (acc, doc) => {
    ...
    if (
      ...
      (uId && ((limit && count === 6) || currentData.complete === "true")) ||
      (likeList && !likeList["likeList"].includes(currentData.docId))
    )
      return;
    currentData["docId"] = doc.id;
    await getBulkImageUrl(currentData.userId, currentData.images)
      .then((response) => (currentData["imagesUrl"] = response))
      .catch((error) => console.error(error));
...
export const postData = async (data) => {
  ...
  dataCorrectImageList["images"] = dataCorrectImageList["images"].map(
    (img) => img.name
  );
  let dataForAlgolia;

  await addDocument(SELLS_DATA, null, dataCorrectImageList)
    .then((response) => {
      dataForAlgolia = response;
      console.log("Document successfully posted!");
    })
    .catch((error) => console.error(error));
  await postImages(data.userId, data["images"])
    .then(() => console.log("Images successfully posted!"))
    .catch((error) => console.error(error));
...
export const fetchUserData = async (uId) => {
  let data = {};
  await getDocumentsOrderedCurrentUser({
    row: "userId",
    comparator: "==",
    givenFilter: uId,
  })
    .then(
      async (fetchedData) =>
        await addImagesToData(fetchedData)
...
export const fetchAllData = async (uId, limit, likeList) => {
  let data = {};
  await getDocumentsOrdered()
    .then(async (fetchedData) => {
      await addImagesToData(fetchedData, uId, limit, likeList)
        .then((response) => (data = response))
        .catch((error) => console.error(error));
    })
    .catch((error) => console.error(error));
